...
const SOCIAL_LINKS = [
  ...
  {
    label: 'WhatsApp',
    href: 'https://wa.me/5511953597007',
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
...
export default function Hero() {
  ...
  // Detecta aparelhos sem mouse (celular/tablet)
  useEffect(() => {
  ...
  // No celular não tem cursor, então o holofote vagueia sozinho pelo fundo
  useEffect(() => {
    if (!isTouch || reduceMotion) return

    const rect = sectionRef.current?.getBoundingClientRect()
    if (rect) {
      // começa no centro sem o sweep inicial vindo de fora da tela
    ...
    const drift = () => {
      ...
      if (r) {
        mouseX.set(r.width * (0.12 + Math.random() * 0.76))
        mouseY.set(r.height * (0.15 + Math.random() * 0.65))
      }
      timeout = setTimeout(drift, 1800 + Math.random() * 1800)
    }
    timeout = setTimeout(drift, 1200)

    return () => clearTimeout(timeout)
  }, [isTouch, reduceMotion, mouseX, mouseY, spotX, spotY])
